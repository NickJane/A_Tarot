...
namespace QConnectSDK.Api
{
    internal  class RequestHelper
    {
        ...
        internal RestRequest CreateTokenRequest(QQConnectConfig config, string code, string state="")
        {
            ...
            Lib.Utils.LogHelper.CustomLog("通过Authorization Code获取Access Token,," +
                string.Format("oauth2.0/token?grant_type=authorization_code&client_id={0}&client_secret={1}&code={2}&redirect_uri={3}", config.GetAppKey(), config.GetAppSecret(), code, config.GetCallBackURI()) + "\r\n");
            return request;
        }
